        ...
        >
          ...
          <div class="p-4">
            <div v-if="product.company" class="flex items-center mb-2">
              ...
              />
              <span class="text-sm text-gray-500 font-medium">{{ product.company.name }}</span>
            </div>
            <h3 class="text-lg font-semibold text-gray-900 mb-2">{{ product.name }}</h3>
            <div v-if="product.sku || product.model_no" class="text-xs text-gray-400 mb-2">
  ...
  setup() {
    ...
    const filteredProducts = computed(() => {
      ...
      if (selectedCategory.value) {
        filtered = filtered.filter(product => String(product.category_id) === String(selectedCategory.value))
      }

      // Filter by brand
      if (selectedBrand.value) {
        filtered = filtered.filter(product => 
          product.company && String(product.company.id) === String(selectedBrand.value)
        )
      }
    ...
    const fetchProducts = async () => {
      try {
        loading.value = true
        error.value = null
        console.log('Fetching products from enhanced API...')
    ...
    const handleImageError = (event) => {
      const productId = event.target.closest('[data-product-id]')?.dataset.productId || Math.floor(Math.random() * 1000)
      event.target.src = `https://picsum.photos/400/300?random=${productId}`
    }
    ...
    // Watchers
    watch([searchQuery, selectedCategory, selectedBrand, sortBy], () => {
      currentPage.value = 1
    })
